...
public class OldCollectionDescriptor : CollectionDescriptor
{
        ...
        : base(factory, type, emitDefaultValues, namingConvention)
    {
        ...
        else if (type.GetInterface(typeof(ICollection<>)) is Type itype)// implements ICollection<T>
        {
            ...
            // implements IList<T>
            if (type.GetInterface(typeof(IList<>)) is Type itype2)
            {
                var insert = itype2.GetMethod(nameof(IList<object>.Insert), [typeof(int), ElementType])!;
                insertMethod = (obj, index, value) => insert.Invoke(obj, [index, value]);
                var removeAt = itype2.GetMethod(nameof(IList<object>.RemoveAt), [typeof(int)])!;
                removeAtMethod = (obj, index) => removeAt.Invoke(obj, [index]);
                var getItem = itype2.GetMethod("get_Item", [typeof(int)])!;
                getIndexedItemMethod = (obj, index) => getItem.Invoke(obj, [index]);
                var setItem = itype2.GetMethod("set_Item", [typeof(int), ElementType])!;
